...
export default function Project() {
  ...
  // UI Showcase Frames (Atas: 1-3, Bawah: 4-6)
  const topFrames = [
  ...
  return (
    ...
    >
      {/* 1. STICKY FEATURED PROJECTS CARDS CONTAINER */}
      <div className="relative w-full h-[135vh] sm:h-[220vh] lg:h-[300vh]">
        <div className="sticky top-0 w-full h-screen flex flex-col justify-start lg:justify-between px-3 sm:px-8 md:px-12 pt-12 sm:pt-20 pb-3 md:pb-6 overflow-hidden">
          ...
          <div className={`max-w-7xl mx-auto w-full z-20 mt-4 sm:mt-8 lg:my-auto transition-all duration-700 ease-out transform delay-[450ms] ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
          }`}>
      ...
      {/* 2. UI SHOWCASE (DIRECTLY INTEGRATED WITH ZERO GAP) */}
      <div className="relative w-full pt-0 sm:pt-8 pb-16 sm:pb-24 overflow-hidden z-20 space-y-5 sm:space-y-8">
        
        {/* ROW 1: TOP ROW (Frame 1-3, Kanan ke Kiri) */}
        <div className="relative w-full overflow-hidden flex select-none group">
          ...
          <div className="flex shrink-0 w-max gap-3.5 sm:gap-6 animate-marquee group-hover:[animation-play-state:paused]">
            {topTrack.map((imgSrc, idx) => (
              <div
                key={`top-${idx}`}
                className="shrink-0 h-40 xs:h-48 sm:h-64 lg:h-72 aspect-[863/473] rounded-xl sm:rounded-2xl overflow-hidden bg-black shadow-2xl transition-all duration-300 transform hover:scale-[1.02]"
              >
                <img
                  src={imgSrc}
        ...
        {/* ROW 2: BOTTOM ROW (Frame 4-6, Kiri ke Kanan) */}
        <div className="relative w-full overflow-hidden flex select-none group">
          <div className="absolute top-0 bottom-0 left-0 w-16 sm:w-36 bg-gradient-to-r from-black to-transparent z-20 pointer-events-none" />
          <div className="absolute top-0 bottom-0 right-0 w-16 sm:w-36 bg-gradient-to-l from-black to-transparent z-20 pointer-events-none" />

          <div className="flex shrink-0 w-max gap-3.5 sm:gap-6 animate-marquee-reverse group-hover:[animation-play-state:paused]">
