...
export default function CertificateSection() {
  ...
  return (
    <section id="certificate" className="relative w-full min-h-screen bg-black text-white px-4 sm:px-8 md:px-16 py-20 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="relative w-full border-t border-dashed border-zinc-700 flex justify-between items-center my-6">
        ...
        <div className="flex justify-center my-8">
          <div className="flex items-center gap-1.5 p-1.5 bg-zinc-900/90 border border-zinc-800 rounded-full backdrop-blur-md shadow-lg">
            {categories.map((cat) => (
              <button
                key={cat}
        ...
        {/* --- GRID CARDS SERAGAM (SETARA) --- */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCerts.map((cert) => (
            <div
              key={cert.id}
              onClick={() => setSelectedCert(cert)}
              className="group relative bg-zinc-900/80 border border-zinc-800 rounded-2xl overflow-hidden backdrop-blur-sm hover:border-fuchsia-500/50 hover:shadow-[0_0_25px_rgba(217,70,239,0.15)] hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col justify-between"
            >
              {/* Gambar Sertifikat */}
              <div className="relative w-full aspect-[16/10] overflow-hidden bg-zinc-950">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center p-4">
                  <span className="text-xs font-semibold bg-fuchsia-600 text-white px-3 py-1.5 rounded-full shadow-md transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 flex items-center gap-1.5">
                    🔍 View Certificate
                  </span>
                </div>

                {/* Badge Kategori */}
                <span className="absolute top-3 left-3 bg-zinc-900/90 text-fuchsia-400 text-[10px] font-mono px-2.5 py-1 rounded-full border border-zinc-700/80 backdrop-blur-md">
                  {cert.category}
                </span>
              </div>
              ...
              <div className="p-5 flex flex-col justify-between flex-grow">
                <div>
                  <div className="flex items-center justify-between text-[11px] text-zinc-500 font-mono mb-1.5">
                    <span>{cert.issuer}</span>
                    <span>{cert.date}</span>
                  </div>
                  <h3 className="text-base font-bold text-white group-hover:text-fuchsia-400 transition-colors mb-2">
                    {cert.title}
                  </h3>
                ...
                <div className="mt-4 pt-3 border-t border-zinc-800/60 flex items-center justify-between text-xs text-zinc-400">
                  <span className="group-hover:text-white transition-colors">Show Credentials</span>
                  <svg className="w-4 h-4 transform group-hover:translate-x-1 transition-transform text-fuchsia-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {selectedCert && (
        ...
        >
          <div 
            ...
            className="relative bg-zinc-900 border border-zinc-800 rounded-2xl max-w-2xl w-full p-5 sm:p-6 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto"
          >
            {/* Tombol Close */}
            <button
              onClick={() => setSelectedCert(null)}
              className="absolute top-4 right-4 w-8 h-8 bg-zinc-800 text-zinc-400 hover:text-white hover:bg-fuchsia-600 rounded-full flex items-center justify-center transition-colors z-10 cursor-pointer"
            >
              ✕
            </button>

            {/* Gambar Besar */}
            <div className="w-full aspect-[16/10] bg-zinc-950 rounded-xl overflow-hidden mb-4 border border-zinc-800">
              <img
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between text-xs font-mono">
                <span className="text-fuchsia-400 bg-fuchsia-500/10 px-2.5 py-0.5 rounded-full border border-fuchsia-500/20">{selectedCert.category}</span>
              ...
              <div className="bg-zinc-950/60 p-3.5 rounded-xl border border-zinc-800/80">
                <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider block mb-1">Deskripsi & Output:</span>
              ...
              <div className="pt-2 flex justify-end">
                <a
                  ...
                  className="px-4 py-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white text-xs font-semibold rounded-lg shadow-lg shadow-fuchsia-600/30 transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer"
                >
